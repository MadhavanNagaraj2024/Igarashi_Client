...
function ExcelTemplate({ triggerPopup }) {
  ...
  const handleSubmit = async () => {
    if (!machine) return triggerPopup("Select a Machine First", "error");

    if (getSelTemplate === "" || getSelTemplate === "select") {
      return triggerPopup("Select a Template Name First", "error");
    }

    if (getExTemplate === "Exists") {
      return triggerPopup("Please Select Another Template Name", "error");
    }
  ...
  const addNewTemplate = async () => {
    if (!machine) {
      return triggerPopup("Select a machine first", "error");
    }
  ...
  const deleteTemplate = async () => {
    ...
    if (getSelTemplate === "") {
      return triggerPopup("Select a template first", "error");
    }
