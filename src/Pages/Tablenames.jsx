...
function Tablenames({ triggerPopup }) {
  ...
  const handleToggleVisibility = async (rowData) => {
    if (rowData?.exportname === "") {
      return triggerPopup("Kindly assign a name to this machine.", "error");
    }
  ...
  const handleRowEdit = (row) => {
    console.log("Row DatazzZZzz", row.original);
  ...
  const hanleEditData = async () => {
    if (getExeUser === "Exists") {
      return triggerPopup("This Name Already Assigned", "error");
    }
    ...
    if (getOldName === "" || getNewName === "") {
      ...
      try {
        ...
        console.log("Error fetching in Rename Table", error);
      }
    }
  };
