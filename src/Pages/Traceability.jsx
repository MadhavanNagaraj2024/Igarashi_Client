...
export default function Traceability(props) {
  ...
  const handleSubmit = async () => {
    if (getDmc === "") {
      return triggerPopup("Select DMC Code", "error");
    }
    try {
      ...
      console.log("Error fetching in DMC Data..?", error);
    }
  };

  const handleExportAll = async () => {
    if (!getTabledataz?.some((t) => t.data?.length > 0)) {
      return triggerPopup("No Data To Export", "error");
    }
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", mb: 2 }}>
          <Button
            ...
            onClick={async () => {
              ...
              if (!res?.success) {
                triggerPopup("Today's folder not found", "error");
              }
            }}
          >
            Open Today Folder
