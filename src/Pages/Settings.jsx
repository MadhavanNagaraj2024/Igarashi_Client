...
export default function Settings({ triggerPopup }) {
  ...
  const handleSubmit = async () => {
    if (getExeUser === "Exists") {
      return triggerPopup("Username already exists", "error");
    }
    ...
    if (
      ...
      return triggerPopup("Please fill the required fields", "error");
      //  alert("Please fill the required fields");
    } else {
      let paramData = getUser;

      paramData.page = getPage?.toString();
  ...
  const debouncedCheck = useCallback(
    debounce(async (value) => {
      if (value.trim() !== "") {
        const res = await window.versions.checkexistuser(value);
  ...
  const handleEdit = (row) => {
    ...
    setPage(row?.original?.page?.split(","));
    setBtnType("edit");
  };
  ...
  const handleUpdate = async () => {
    ...
    if (
      ...
      const res = await window.versions.updateuser(queryStr);
  ...
  const handleDelete = async (row) => {
    ...
    `;
        ...
        const result = await window.versions.deleteuser(querystr);

        if (result.message === "User deleted successfully" && result.success) {
          getUSersDeatils();
            ...
            <Grid item sm={8} mt={2}>
              <TextField
                ...
                onChange={async (e) => {
                  ...
                  setUser({ ...getUser, username: value });
